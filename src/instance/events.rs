...
pub fn handle_mouse_event(state: &mut InstanceState, mouse: MouseEvent) {
    match mouse.kind {
        MouseEventKind::ScrollUp | MouseEventKind::ScrollDown => {
            if let Some(render_area) = state.last_render_area {
                let pane_areas = super::layout::calculate_pane_areas(
                    render_area,
                    state.layout_mode,
                    state.panes.len(),
                );
                ...
                for (index, area) in pane_areas.iter().enumerate() {
                    ...
                    {
                        if let Some(pane) = state.panes.get_mut(index) {
                            if let Some(session) = &mut pane.pty_session {
                                const SCROLL_LINES: u8 = 3;
                                let scroll_data = match mouse.kind {
                                    MouseEventKind::ScrollUp => {
                                        format!("\x1b[{}A", SCROLL_LINES).into_bytes()
                                    }
                                    MouseEventKind::ScrollDown => {
                                        format!("\x1b[{}B", SCROLL_LINES).into_bytes()
                                    }
                                    _ => return,
                                };
