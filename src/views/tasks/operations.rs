...
use super::state::{Column, FocusPanel, Task, TaskType, TasksMode, TasksState};
use uuid::Uuid;
...
impl TasksState {
    ...
    pub fn start_classification_for_edit(&mut self, raw_input: String, task_id: Uuid) {
        self.start_classification_internal(raw_input, Some(task_id));
    }
    ...
    pub fn edit_kanban_task(&mut self, task_index: usize, title: String, description: String) {
    ...
    pub fn delete_kanban_task(&mut self, task_index: usize) {
        ...
        if task_index < column.tasks.len() {
            ...
            if column.tasks.is_empty() {
                ...
                self.kanban_selected_task = Some(column.tasks.len() - 1);
            }
        }
    }
    ...
    fn try_merge_worktree(&self, task: &Task) -> Option<crate::views::worktree::MergeResult> {
    ...
    fn try_cleanup_worktree(&self, task: &Task) {
    ...
    pub fn find_task_index_by_id(&self, task_id: Uuid) -> Option<usize> {
        let review_column_index = 2;
        self.columns
            .get(review_column_index)?
            .tasks
    ...
    pub fn get_focus_selected_task(&self) -> Option<TaskReference<'_>> {
        let active_tasks = get_active_tasks(&self.columns);
        let done_tasks = get_done_tasks(&self.columns);

        match self.focus_panel {
            FocusPanel::ActiveTasks => active_tasks.into_iter().nth(self.focus_active_index),
            FocusPanel::DoneTasks => done_tasks.into_iter().nth(self.focus_done_index),
        }
    }
}
