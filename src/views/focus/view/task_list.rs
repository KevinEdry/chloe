...
pub fn render(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let total_task_count: usize = columns.iter().map(|c| c.tasks.len()).sum();
    ...
    if total_task_count > 0 {
        let position_text = format!(" {} of {} ", state.selected_index + 1, total_task_count);
    ...
    let items = build_task_list_items(app, active_task_count);
...
fn build_task_list_items(app: &App, active_task_count: usize) -> Vec<ListItem<'static>> {
    let mut items = Vec::new();
    let mut current_index = 0;
    let columns = &app.kanban.columns;
    let selected_index = app.focus.selected_index;
    ...
    if let Some(done_column) = columns.get(3) {
        if !done_column.tasks.is_empty() {
            ...
            items.push(create_column_header(&done_column.name, 3));
...
fn create_task_item(
    ...
    let mut spans = vec![
        ...
        Span::styled(
            ...
            Style::default()
                .fg(title_color)
                .add_modifier(if is_selected && !is_done {
                    Modifier::BOLD
                } else {
                    Modifier::empty()
                }),
